...
export const vllmParams: ParamDef[] = [
  ...
  { key: "gpuMemUtil", label: "显存利用率", labelEn: "GPU memory util", type: "number", default: 0.8, step: 0.05, min: 0, tab: "gpu", flag: "--gpu-memory-utilization", desc: "GPU 显存使用比例（0–1）；默认 0.8 对 8GB 卡更稳妥（WSL2/驱动占用会吃掉约 1GB，0.9 易触发显存不足）", descEn: "Fraction of GPU VRAM to use (0–1); default 0.8 is safer on 8GB cards (WSL2/driver overhead eats ~1GB, 0.9 can hit out-of-memory)" },
